...
def time_wav_setting_submit(entry1,entry2,entry3,window):
...
def save_settings_to_file(path):
...
def input_level_adjustment_window(test_signal,duration):
...
def analysis():
    ...
    if frequency_spectrum_var.get()==True:
        n_fft=2048
...
def recording_window(level_window):
    ...
    def recording():
        ...
        if cnt < len(parameters):  # レコーディング処理 
            ...
            recorded_signal=sd.playrec(signal,samplerate=sr,channels=1,dtype=np.int16)
            sd.wait()
            for i in range((10+1)*sr):
                if(recorded_signal[i]!=0):
                    break
            rec_time=sr*10
            sliced_signal=recorded_signal[i:i+rec_time]
            recording_data.append(sliced_signal)
            cnt += 1
...
def n_times_recording_window(level_window):
    level_window.destroy()
    recording_window = tk.Toplevel(root)
    recording_window.title("Experimental-App")
    recording_window.geometry("800x400")
        ...
        if cnt < int(rec_times_entry.get()):  # レコーディング処理 
            ...
            for i in range((10+1)*sr):
                if(recorded_signal[i]!=0):
                    break
            signal_length=sr*10
            sliced_signal=recorded_signal[i:i+signal_length]
            recording_data.append(sliced_signal)
            cnt += 1
